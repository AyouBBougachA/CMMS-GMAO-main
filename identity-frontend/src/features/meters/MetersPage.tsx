...
import { Activity, Plus, Search, Loader2, Thermometer, Gauge, Clock, AlertTriangle } from 'lucide-react';
          ...
          {filteredMeters.map((meter) => (
            <Card key={meter.meterId} className="bg-card hover:shadow-md transition-all border-border/60 overflow-hidden group">
              <CardHeader className="pb-3 border-b border-border/40 bg-muted/20">
                ...
                <CardTitle className="text-lg mt-3">{meter.name}</CardTitle>
              ...
              <CardContent className="pt-5 space-y-4">
                ...
                <div className="pt-4 flex gap-2">
                  ...
                  <Button className="flex-1 text-xs h-9" onClick={() => handleUpdateClick(meter)}>Update</Button>
                </div>
              </CardContent>
            </Card>
          ))}
